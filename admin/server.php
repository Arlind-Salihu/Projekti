...
if (isset($_POST['reg_user'])) {
 ...
 // së pari kontrolloni bazën e të dhënave për tu siguruar
  ...
  $user_check_query = "SELECT * FROM perdoruesi1 WHERE emri='$emri' OR email='$email' LIMIT 1";
...
// Në fund, regjistroni përdoruesin nëse nuk ka gabime në formë
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO perdoruesi1 (emri, email, flk) 
...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
  	$flk = md5($flk);
  	$query = "SELECT * FROM perdoruesi1 WHERE emri='$emri' AND flk='$flk'";
