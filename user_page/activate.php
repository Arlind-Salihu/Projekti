	...
	if(!isset($_GET['code']) OR !isset($_GET['user'])){
		$output .= '
			<div class="alert alert-danger">
                ...
                Kodi për të aktivizuar llogarinë nuk u gjet..
            </div>
            <h4>Ti mund <a href="signup.php">Signup</a> ose përsëri në<a href="index.php">Fillim</a>.</h4>
		'; 
	}
	else{
		$conn = $pdo->open();
		...
		if($row['numrows'] > 0){
			if($row['status']){
				$output .= '
					<div class="alert alert-danger">
		                <h4><i class="icon fa fa-warning"></i> Error!</h4>
		               Llogaria tashmë është aktivizuar.
		            </div>
		            <h4>Ti mund <a href="signup.php">Signup</a> ose përsëri në<a href="index.php">Fillim</a>.</h4>
				';
			}
